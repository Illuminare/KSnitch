...
namespace KSnitch.Analysis
{
    internal sealed class ProjectBuilder
    {
            ...
            bool simplifiedOutput = false)
        {
            ...
            var project = Build(manager, path, tfm, skip, built ,0, simplifiedOutput);
            ...
            bool simplifiedOutput = false)
        {
            ...
            var result = Build(manager, project, tfm, indentation, simplifiedOutput);
            if (result == null)
            {
            ...
            foreach (var projectReference in result.ProjectReferences)
            {
                ...
                var analyzedProjectReference = Build(manager, projectReferencePath, project.TargetFramework, skip, built, indentation + 1, simplifiedOutput);
        ...
        private IAnalyzerResult? Build(AnalyzerManager manager, Project project, string? tfm, int indentation, bool simplifiedOutput = false)
        {
            var prefix = new string(' ', indentation * 2);
            if (indentation > 0)
            {
